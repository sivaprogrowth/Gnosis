...
1a. **Check the web drain queue** — Supabase project `jqpyzkqtnzzrhemnyegk`, table `public.gnosis_drain_queue` (`SELECT readwise_book_id, book_title, requested_class, requested_at FROM gnosis_drain_queue WHERE status = 'pending' ORDER BY requested_at ASC`). These are books the user queued from the gnosis.progrowth.services Library UI. Bring them to the top of the drain list and **default each to its `requested_class`** (almost always 'A' — the UI defaults to A because queueing is an explicit user intent). After processing, update the row to `status = 'drained'` and `drained_at = now()`. If the user later overrides the class, update `requested_class` too.
2. **Summarize each in one line** — title, author, highlight count, date added. Optionally pull 2–3 representative highlights to ground the recommendation. Mark web-queued books with a `[queued via web]` tag in the summary so the user sees their own request reflected. Example: *"The Creative Act — Rick Rubin (138 highlights, added 2024-03-12) — taste-as-craft and the silent practice of choosing what's true."*
3. **Recommend a triage class for each** — one of:
   - **A) Full book ingest** — the §4.1 9-step workflow, scaled for books. Recommend when the book introduces named frameworks, has heavy marginalia, or sits adjacent to active project work. Note expected page touch count. **Default for web-queued books** unless a specific signal suggests otherwise.
