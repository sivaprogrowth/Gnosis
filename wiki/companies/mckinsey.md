---
type: company
title: "McKinsey"
tags: [ingested]
sources: ["ai-native-telco-radical-transformation"]
---

# McKinsey

_Stub page created automatically during ingest of [[ai-native-telco-radical-transformation|The AI-native telco: Radical transformation to thrive in turbulent times]]._

McKinsey is the authoring organisation of this source and is referenced across multiple existing sources (b2b-pulse-survey, ai-transformation, jagged-frontier, big-brand-bias, etc.). It fits the pattern of companies the wiki clearly tracks and would be cited by many existing pages.

## Why this earned its own page

McKinsey is the authoring organisation of this source and is referenced across multiple existing sources (b2b-pulse-survey, ai-transformation, jagged-frontier, big-brand-bias, etc.). It fits the pattern of companies the wiki clearly tracks and would be cited by many existing pages.

## Mentions


- [[what-is-central-bank-digital-currency-cbdc|What is central bank digital currency (CBDC)?]]
- [[ai-native-telco-radical-transformation|The AI-native telco: Radical transformation to thrive in turbulent times]]
