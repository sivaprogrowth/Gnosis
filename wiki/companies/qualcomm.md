---
type: company
title: "Qualcomm"
tags: [ingested]
sources: ["wsj-beyond-smartphone-mwc-2023"]
---

# Qualcomm

_Stub page created automatically during ingest of [[wsj-beyond-smartphone-mwc-2023|The New Tech That Will Take Us Beyond the Smartphone. Maybe.]]._

Multiple existing pages discuss AI semiconductor supply chain, on-device AI, and related hardware topics (concept/concepts/ai-semiconductor-supply-chain, concept/companies/nvidia, concept/companies/tsmc, concept/companies/sk-hynix). Qualcomm is a named chip company that would naturally be cited across semiconductor and on-device AI discussions.

## Why this earned its own page

Multiple existing pages discuss AI semiconductor supply chain, on-device AI, and related hardware topics (concept/concepts/ai-semiconductor-supply-chain, concept/companies/nvidia, concept/companies/tsmc, concept/companies/sk-hynix). Qualcomm is a named chip company that would naturally be cited across semiconductor and on-device AI discussions.

## Mentions


- [[how-jensen-huang-became-ais-taylor-swift|How Jensen Huang became AI’s Taylor Swift]]
- [[wsj-beyond-smartphone-mwc-2023|The New Tech That Will Take Us Beyond the Smartphone. Maybe.]]
